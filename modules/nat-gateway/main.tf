...
resource "aws_nat_gateway" "nat_gateway_az1" {
  ...
  # use the name on the outputs.tf 
...
# We will this route tabe tp route traffic to tne nat gateway in the public subnet az1
resource "aws_route_table" "private_route_table_az1" {
  vpc_id = var.vpc_id

  route {
    cidr_block = "0.0.0.0/0"
    # this is the nat gateway to route traffic to the internet in the public subnet az1
...
# Route traffic to the nat nat gateway in the second avz
